...
function toggleexpander(blockid, arrowid) {
  ...
  if (block.style.display == "none") {
    ...
    if (arrow.src.indexOf("/") >= 0) {
      arrow.src =
        arrow.src.substring(0, arrow.src.lastIndexOf("/") + 1) +
        "arrow_down.gif";
    } else {
...
function writeDocFeedbackSurveyLink() {
  ...
  code =
    ...
    "<strong>" +
    getLocalizedString("was_this_topic_helpful") +
...
$(document).ready(function () {
  ...
  if (getParameterByName("browser") === "F1help") {
    ...
    $("body#responsive_offcanvas")
      .addClass("navigation_off")
      .addClass("no_animate");
    $(".site_container:first")
...
function applySearchHighlight() {
  if ($.fn.highlight) {
    ...
    if (searchHighlightTerm.length > 0) {
      var searchHighlightArray =
        getParameterByName("searchHighlight").match(/"[^"]+"|\S+/g);
  ...
  $.each(elements, function () {
    if (!$(this).is(":visible")) {
      ...
      if (collapsedParent.length > 0) {
        ...
        if (
          collapsedParent.hasClass("expand") &&
    ...
    if (target.length > 0) {
      $(this).click(function (evt) {
        ...
        $(scrollParameter).animate({ scrollTop: scrollTop }, 700, function () {
          ...
          if (
            nextSibling.hasClass("expand") &&
            !nextSibling.hasClass("expanded")
          ) {
            doToggle(nextSibling);
          }
        });
...
function doExpand(elt, noAnimation) {
  ...
  if (
    (browserInfo.name === "msie" && browserInfo.version <= 8) ||
    noAnimation
  ) {
...
function getScrollParameter() {
  ...
  var browserInfo = $.browserInfo();
  var scrollParameter =
    browserInfo.name === "msie" || browserInfo.name === "mozilla"
      ? "html"
      : "body";
