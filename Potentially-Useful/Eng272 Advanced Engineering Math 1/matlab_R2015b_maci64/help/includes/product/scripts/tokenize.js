...
(function ($) {
  ...
  var create_token = function (parent, searchInput, options) {
    ...
    token.append(
      $(
        '<a href="#remove-token"class="icon-remove" title="Remove: ' +
          options.label +
          '"><span class="label">' +
          options.label +
          "</span></a>"
  ...
  $.fn.tokenize = function (options) {
    ...
    this.closest("form")
      ...
      .bind("submit", function (event) {
        var term =
          input.val() +
          " " +
          tokens_to_query(nugget_box.find(".nugget").not(".nugget_remove_all"));
        term = $.trim(term);
    ...
    $.each(parsed_query.tokens, function (i, token) {
      create_token(tokens, input, {
        field: token.field,
        value: token.value,
        label: options.label_function
          ? options.label_function(token)
          : token.value,
      });
    });
    ...
    if (parsed_query.tokens.length > 1) {
      // Create remove link
      var $remove_all = $(
        '<span class="nugget nugget_remove_all"><a href="#" title="Remove All"><span class="label">Remove All</span></a></span>'
      );
