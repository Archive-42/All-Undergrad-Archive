...
function Suggestions(retrievalFunction, moreFunction, searchField) {
  ...
  this.retrieveMore = function (type) {
    var text = suggestionsObj.searchField.val();
    document.location =
      "docsuggestion:" + type + "|" + encodeURIComponent(text);
  };
...
Suggestions.prototype.formatSuggestions = function () {
  ...
  if (
    !(
      (this.suggestions.pages && this.suggestions.pages.length > 0) ||
  ...
  suggestionsElt
    .find(".suggestion-more")
    .on("selection-action", function (evt) {
      evt.stopPropagation();
      suggestionsObj.more($(this).data("pagetype"));
    });

  suggestionsElt
    .find(".suggestionarea:not([wordarea])")
    .on("selection-action", function () {
...
Suggestions.prototype.getSuggestionsUrl = function (action, params) {
  ...
  return (
    formAction.replace(/\/search(\/|$)/, "/search/" + action + "/") +
    "?" +
    $.param(params)
  );
};
...
Suggestions.prototype.displayMoreSuggestions = function (
  type,
  template,
  suggestions
) {
...
function getNextSelection() {
  ...
  if (selected.length > 0) {
    ...
    if (newselection.length == 0) {
      ...
      var area = selected
        .closest(".suggestionarea")
        .nextAll(".suggestionarea:first");
